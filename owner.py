...
class Owner(commands.Cog):
    ...
    async def unload(self, ctx, extencion):
        if  extencion.startswith('owner'):
            await ctx.send(f'<@{ctx.message.author.id}> Nu îți este permis să dezactivezi modulul `{extencion}`.')
        else:
            self.client.unload_extension(f'cogs.{extencion}')
            await ctx.send(f'```Modulul {extencion} a fost descărcat cu succes.```')
            global cogsloaded
    ...
    async def reload(self, ctx, extencion):
        if extencion.startswith('owner'):
            await ctx.send(f'<@{ctx.message.author.id}> Nu îți este permis să reîncarci modulul `{extencion}`.')
        else:
            self.client.unload_extension(f'cogs.{extencion}')
            self.client.load_extension(f'cogs.{extencion}')
            await ctx.send(f'```Modulul {extencion} a fost reîncărcat cu succes.```')
    ...
    async def reloadall(self, ctx):
        ...
        await ctx.send(f'```Toate modulele au fost reîncărcate cu succes.```')
        global cogsloaded
        cogsloaded.clear()
        for filename in os.listdir('./cogs'):
            if filename.endswith('.py'):
    ...
    async def allmodules(self, ctx):
        ...
        embed.set_author(name='Owner: rootblind' ,icon_url= 'https://cdn.discordapp.com/avatars/{0.id}/{0.avatar}.png?size=1024'.format(ctx.message.author))
        counter = int(0)
        for x in cogsloaded:
            counter += 1
    ...
    async def load_error(self, ctx, error):
        if isinstance(error, commands.errors.CheckFailure):
            ...
            await ctx.send(
                f'<@{ctx.message.author.id}> modulul `{ctx.message.content.split(" ")[1]}` nu există sau are erori.')
    ...
    async def allmodules_error(self, ctx, error):
        if isinstance(error, commands.errors.CheckFailure):
            ...
            await ctx.send(f'<@{ctx.message.author.id}> a apărut o eroare.')
